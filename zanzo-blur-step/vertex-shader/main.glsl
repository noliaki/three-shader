...
void main(void) {
  ...
  float nNoise = (noise + 1.0) * 0.5;
  float offsetNoise = (snoise(vec3(position.xy, uTime * 0.008)) + 1.0) * 0.5;
  // float sizeOffset = step(sin(uTime * 0.01 * nNoise), 0.1);

  gl_PointSize = 1.0 + nNoise * 50.0 + pow(offsetNoise, 8.0) * 150.0;
